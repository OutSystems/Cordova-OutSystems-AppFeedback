module.exports = function (ctx) {
    ...
    var osMobileECTPbxFileRef = findPbxFileReference(proj, "OutSystemsMobileECT.framework");
    // Clean extra " on the start and end of the string
    var osMobileECTPbxFileRefPath = osMobileECTPbxFileRef.path;
    if (osMobileECTPbxFileRefPath.endsWith("\"")) {
        osMobileECTPbxFileRefPath = osMobileECTPbxFileRefPath.substring(0, osMobileECTPbxFileRefPath.length - 1);
    }
    if (osMobileECTPbxFileRefPath.startsWith("\"")) {
        osMobileECTPbxFileRefPath = osMobileECTPbxFileRefPath.substring(1, osMobileECTPbxFileRefPath.length);
    }
    ...
    var removedPbxFile = proj.removeFramework(osMobileECTPbxFileRefPath, {
        customFramework: true
    });
    // Re-add the framework but with embed
    var addedPbxFile = proj.addFramework(osMobileECTPbxFileRefPath, {
        customFramework: true,
        embed: true,
        sign: true
    });
